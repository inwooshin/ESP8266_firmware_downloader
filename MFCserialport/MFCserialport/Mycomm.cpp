...
UINT CommThread(LPVOID lpData)
{
	...
	Commy = (CMycomm*)lpData;

	while (Commy->m_bIsOpenned)
	{
		EvtMask = 0;
		Length = 0;
		insize = 0;
		memset(buf, '\0', MAXBUF);
		WaitCommEvent(Commy->m_hComDev, &EvtMask, NULL);
		ClearCommError(Commy->m_hComDev, &ErrorFlags, &ComStat);
		if ((EvtMask & EV_RXCHAR) && ComStat.cbInQue)
		{
			...
			{
				ClearCommError(Commy->m_hComDev, &ErrorFlags, &ComStat);
				if (!ReadFile(Commy->m_hComDev, buf + insize, size, &Length, &(Commy->m_OLR)))
				{
					// ¿¡·¯
					TRACE("Error in ReadFile\n");
					if (GetLastError() == ERROR_IO_PENDING)
					{
						if (WaitForSingleObject(Commy->m_OLR.hEvent, 1000) != WAIT_OBJECT_0)
							Length = 0;
						else
							GetOverlappedResult(Commy->m_hComDev, &(Commy->m_OLR), &Length, FALSE);
					}
					else
						Length = 0;
				}
			...
			if (Commy->m_nLength + insize > MAXBUF * 2)
				insize = (Commy->m_nLength + insize) - MAXBUF * 2;

			Commy->m_pEvent->ResetEvent();
			memcpy(Commy->m_sInBuf + Commy->m_nLength, buf, insize);
			Commy->m_nLength += insize;
			Commy->m_pEvent->SetEvent();
			LPARAM temp = (LPARAM)Commy;
			SendMessage(Commy->m_hWnd, WM_MYRECEIVE, Commy->m_nLength, temp);
		}
	}
